...
class RTMMCPServer
  ...
  def handle_tools_call(request)
    ...
    when 'test_connection'
      handle_test_connection(id, arguments)
    when 'list_all_lists'
      handle_list_all_lists(id, arguments)
    when 'create_list'
      handle_create_list(id, arguments)
    when 'list_tasks'
      handle_list_tasks(id, arguments)
    when 'create_task'
      handle_create_task(id, arguments)
    when 'complete_task'
      handle_complete_task(id, arguments)
    else
      {
        "jsonrpc" => "2.0",
        "error" => {
  ...
  def handle_test_connection(id, arguments)
  ...
  def handle_list_all_lists(id, arguments)
  ...
  def handle_create_list(id, arguments)
    name = arguments['name']
  ...
  def handle_list_tasks(id, arguments)
  ...
  def handle_create_task(id, arguments)
    name = arguments['name']
    list_id = arguments['list_id']
        ...
        list_name = list['name'] || 'Unknown list'
        {
          ...
          "result" => {
            "content" => [
              {
                "type" => "text",
                "text" => "✅ Created task: #{task_name} in #{list_name}"
              }
            ]
          },
          "id" => id
  ...
  def handle_complete_task(id, arguments)
    list_id = arguments['list_id']
